...
def main():
    ...
    # Define time range based on particle type
    if particle_type == "axion":
        period = 2 * np.pi / omega_a / s_to_eVminus1
        ts = np.linspace(0, 4 * period, 1000)
        m = m_a
    elif particle_type == "dark photon":
        period = 2 * np.pi / omega_d / s_to_eVminus1
        ts = np.linspace(0, 4 * period, 1000)
        m = m_d

    # Print the period of oscillation (yr)
    ...
    # Hydrogen parameters for weak-field Zeeman splitting
